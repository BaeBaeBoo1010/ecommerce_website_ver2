...
export default function AddProductClient({ initialCategories }: Props) {
  ...
  // Use initialCategories if available, otherwise fetch or fallback
  // Actually, if we pass initialCategories, we might not need useSWR for categories at all,
  // unless we want to support "Add new category" reflecting immediately without reload?
  // The existing code has `mutate` for adding category, so we should keep useSWR or local state.
  // Let's use useSWR with validation disabled, but seeding with initial data.
