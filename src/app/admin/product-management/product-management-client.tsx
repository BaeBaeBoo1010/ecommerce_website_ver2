...
export default function ProductManagementClient({
  ...
  const {
    ...
    fallbackData: initialProducts, // Use server-provided initial data
    revalidateOnMount: false, // Don't re-fetch immediately
  ...
  const { data: categories = [] } = useSWR<Category[]>(
    ...
    fetcher,
    {
      fallbackData: initialCategories, // Use server-provided initial data
      revalidateOnMount: false, // Don't re-fetch immediately
      revalidateOnFocus: false,
      revalidateOnReconnect: false,
    },
                        ...
                        >
                          ...
                          >
                            <div className="border-border/50 bg-muted/30 relative aspect-square h-14 w-14 overflow-hidden rounded-lg border shadow-sm transition-all group-hover:ring-2 group-hover:ring-blue-500">
                              <Image
                                src={
                                  p.imageUrls[0] || "/images/placeholder.svg"
                                }
                                alt={p.name}
                                fill
                                unoptimized
                          ...
                          <TableCell
                            className="w-32 cursor-pointer text-sm font-medium break-words whitespace-normal transition-colors group-hover:text-blue-600 dark:group-hover:text-blue-400"
                            onClick={() => router.push(`/products/${p.slug}`)}
                          >
                            {p.category?.name}
                          </TableCell>
